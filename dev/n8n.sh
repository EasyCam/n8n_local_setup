...
install_nodejs() {
  log "安装 Node.js 20.x (使用淘宝镜像)..."

  # 方法1：使用 NodeSource 但通过淘宝镜像
  # 首先尝试安装 NodeSource GPG key
  curl -fsSL https://deb.nodesource.com/gpgkey/nodesource.gpg.key | gpg --dearmor -o /usr/share/keyrings/nodesource.gpg || {
    warn "NodeSource GPG key 下载失败，尝试备用方案..."
  }

  # 如果 NodeSource 不可用，使用 snap 安装
